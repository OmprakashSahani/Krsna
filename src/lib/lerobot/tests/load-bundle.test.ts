...
import { decodeManifest } from "@/lib/lerobot/atlas-schema/validate";
import {
  loadDemoBundle,
...
describe("bundle loading", () => {
  ...
  it("uses demo-v2 by default and validates local bundle overrides", () => {
    expect(resolveBundleBase(undefined)).toBe("/lerobot-state-atlas/demo-v2");
    expect(resolveBundleBase("/lerobot-state-atlas/__local-benchmark__")).toBe(
      "/lerobot-state-atlas/__local-benchmark__",
    );
    expect(resolveBundleBase("/lerobot-state-atlas/__local-benchmark__/")).toBe(
      "/lerobot-state-atlas/__local-benchmark__",
    );
  ...
  it.each([
    "",
    "https://example.com/lerobot-state-atlas/pilot",
    "//example.com/lerobot-state-atlas/pilot",
    "/tmp/pilot",
    "/lerobot-state-atlas/../demo-v2",
    "/lerobot-state-atlas/pilot\\bundle",
    "/lerobot-state-atlas/pilot?version=1",
    "/lerobot-state-atlas/pilot#fragment",
  ])("rejects unsafe bundle override %j", (override) => {
    expect(() => resolveBundleBase(override)).toThrow(
      /safe root-relative \/lerobot-state-atlas\/ path/,
    );
  });
  ...
  it("applies one override to initial and lazy payload requests", async () => {
    ...
    expect(urls).toEqual([
      "/lerobot-state-atlas/__local-benchmark__/manifest.json",
      "/lerobot-state-atlas/__local-benchmark__/coverage.json",
      "/lerobot-state-atlas/__local-benchmark__/trajectories.json",
    ]);
  ...
  it("loads development manifest and coverage with no-store", async () => {
    ...
    expect(urls).toEqual([
      "/lerobot-state-atlas/demo-v2/manifest.json",
      "/lerobot-state-atlas/demo-v2/coverage.json",
    ]);
  ...
  it("lazily requests episode-video metadata with the trajectory cache policy", async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith(
      "/lerobot-state-atlas/demo-v2/episode-videos.json",
      { cache: "no-store" },
    );
