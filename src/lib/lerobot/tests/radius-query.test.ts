...
import type { CoveragePayload, PreparedVoxelArm } from "@/lib/lerobot/atlas-schema/types";
import { queryRadius } from "@/lib/lerobot/data/radiusQuery";
import { scoreUncommonEpisodes } from "@/lib/lerobot/data/uncommonEpisodes";
...
describe("shared-world radius queries", () => {
  it.each([
    { radius: 0.125, entries: 2 },
    { radius: 0.125 - 5e-8, entries: 2 },
    { radius: 0.125 - 2e-7, entries: 1 },
    { radius: 0, entries: 1 },
  ])("preserves inclusive boundary and positive-radius epsilon at $radius", ({ radius, entries }) => {
    const arms = structuredClone(prepared);
    arms[0].centers = Float32Array.from([0, 0, 0, 0.125, 0, 0]);
    arms[1].centers = Float32Array.from([2, 2, 2, 3, 3, 3]);
    const selected = { ...selection, exportedCenter: [0, 0, 0] as [number, number, number] };
    expect(queryRadius(arms, coverage, selected, radius, 0.8, 0.8).entryCount).toBe(entries);
  });

  it("does not apply the positive-radius epsilon to a zero-radius query", () => {
    const arms = structuredClone(prepared);
    arms[0].centers = Float32Array.from([0, 0, 0, 5e-8, 0, 0]);
    arms[1].centers = Float32Array.from([2, 2, 2, 3, 3, 3]);
    const selected = { ...selection, exportedCenter: [0, 0, 0] as [number, number, number] };
    expect(queryRadius(arms, coverage, selected, 0, 0.8, 0.8).entryCount).toBe(1);
  });
