...
import type {
  RadiusQueryResult,
  VoxelSelection,
} from "@/lib/lerobot/data/radiusQuery";
  ...
  return (
    ...
    >
      ...
      <section
        ...
        className={styles["uncommon-episodes"]}
      >
        <div className={styles["episode-analysis-context"]}>
          ...
          {episodeCount === 1 ? (
            <p className={styles["uncommon-special-state"]} role="note">
              Relative uncommonness is unavailable with one coverage episode;
              scores are defined as zero.
            </p>
          ) : null}
          <p
            className={styles["uncommon-result-summary"]}
            aria-atomic="true"
            aria-live="polite"
          >
          ...
          {trajectories.status === "idle" ? (
            <button
              className={styles["compact-button"] + " " + styles["uncommon-playback-check"]}
              type="button"
          ...
          {trajectories.status === "loading" ? (
            <p className={styles["uncommon-playback-state"]} aria-busy="true">
        ...
        <div className={styles["episode-analysis-results"]}>
          {scores.length === 0 && usesRadiusScope ? (
            <p className={styles["uncommon-special-state"]}>
              No episode evidence exists in the current radius.
            </p>
          ) : (
            <ol
              className={styles["uncommon-episode-list"]}
              aria-label={`Uncommon-space episode ranking for ${scopeLabel}`}
            >
              {scores.map((result) => (
                <li key={result.episodeId}>
                  <div className={styles["uncommon-episode-heading"]}>
                  ...
                  <div className={styles["uncommon-playback-availability"]}>
                    {trajectories.status === "idle" ? (
